...
err::result<display> display::ctor(environment& env, glfw::window& t_window, const display_create_info& t_create_info)
{
	display res{};
	...
	const u32 image_count = std::clamp(t_create_info.desired_image_count, surface_capabilities.minImageCount, surface_capabilities.maxImageCount);

	res.m_image_format = surface_formats[0].format;
	res.m_image_color_space = surface_formats[0].colorSpace;

	if (std::ranges::any_of(surface_present_modes, [](const vk::PresentModeKHR& mode) { return mode == vk::PresentModeKHR::eFifo; }))
		res.m_present_mode = vk::PresentModeKHR::eFifo;

	const u32 width = std::clamp(
			surface_capabilities.currentExtent.width,
			surface_capabilities.minImageExtent.width,
			surface_capabilities.maxImageExtent.width);
	const u32 height = std::clamp(
	...
	std::vector<u32> family_indices{ u32(env.graphics_family()) };
...
void display::recreate()
{
	...
	vk::SurfaceCapabilitiesKHR surface_capabilities = m_environment->phys_device().getSurfaceCapabilitiesKHR(*m_surface);
	...
	std::vector<u32> family_indices{ u32(m_environment->graphics_family()) };
	vk::SwapchainCreateInfoKHR swapchain_create_info(
			{},
			*m_surface,
			n_images,
